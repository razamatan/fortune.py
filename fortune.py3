...
def get_random_fortune(fortunepaths, weighted=True, offensive=None, 
    ...
    while True:
        ...
        or (max_length is not None and length > max_length)): 
            attempt += 1
            if attempt > ATTEMPTS:
                print("I've given up on finding a fortune that matches your " \
                        "criteria. They are too strict.", file=sys.stderr)
                return ""
            continue
        ffh = open(fortune_file, 'r')
        ffh.seek(start)
...
def check_percentages(percentages):
    ...
    try:
        ...
        print("Percentages are in incompatible formats", file=sys.stderr)
        sys.exit(1)
    if percentage_sum > 100.0:
        print("Percentages add up to more than a hundred", file=sys.stderr)
        sys.exit(1)
    ...
    if len(percentages) == len(given_values): # no None-values
        ...
        try:
            ...
            print("Percentages cannot be converted to float", file=sys.stderr)
            sys.exit(1)
    else:
        for i, percentage in enumerate(percentages):
            if percentage is not None:
                try:
                    ...
                    print("Percentages cannot be converted to float",
                            file=sys.stderr)
                    sys.exit(1)
    return percentages
...
def filter_fortunes(fortunepaths, pattern, ignorecase=True, offensive=None,
    ...
    while not found_first_match:
        ...
        print("(" + os.path.split(fortune_file)[1] + ")", file=sys.stderr)
        print("%", file=sys.stderr)
        sys.stderr.flush()
        fortunes = read_fortunes(open(fortune_file, 'r'))
        while not found_first_match:
            try:
...
def make_fortune_data_file(fortunepaths, quiet=False):
    ...
    for fortune_file in fortune_files:
        ...
        longest = 0
        for start, length, fortune in read_fortunes(open(fortune_file, 'r')):
            data += [(start, length)]
